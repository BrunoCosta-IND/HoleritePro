...
BEGIN
    ...
    RETURN (digito1 = substring(cpf_limpo, 10, 1)::INTEGER AND 
            digito2 = substring(cpf_limpo, 11, 1)::INTEGER);
END;
$$ LANGUAGE plpgsql;
...
-- Constraints para CPF válido
ALTER TABLE usuarios ADD CONSTRAINT check_cpf_valido CHECK (validar_cpf(cpf));
ALTER TABLE funcionarios ADD CONSTRAINT check_cpf_valido CHECK (validar_cpf(cpf));
ALTER TABLE holerite ADD CONSTRAINT check_cpf_valido CHECK (validar_cpf(cpf));
...
-- Constraints para WhatsApp válido
...
-- Nota: O bucket 'holerites' deve ser criado manualmente no painel do Supabase
-- com as seguintes configurações:
-- - Nome: holerites
-- - Público: false
-- - Política de retenção: conforme necessário
...
- Funcionário: 12345678900 / 123456
*/ 